...
import { updateOwnStaffProfile } from '@/api/staff.api';
...
export default function StaffEditProfilePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (ngaysinh) payload.ngaysinh = ngaysinh.toISOString(); // kept in local update state

      // Send using alias `ngaysinhnv` on the API layer
      await updateOwnStaffProfile(user.manvyt, payload);

      // Update local session user and force reload so AuthProvider re-reads
      const updatedUser = {
        ...user,
        ...payload,
      ...
      setSuccess(true);
      setTimeout(() => {
        window.location.href = '/dashboard/profile/staff';
      }, 1000);
    } catch (err: any) {
      ...
      <div className="relative backdrop-blur-lg bg-white rounded-3xl p-8 shadow-2xl overflow-hidden border-t-4 border-green-500">
        ...
        <div className="relative z-10">
          <div className="flex items-center gap-4">
            <Link
              href="/dashboard/profile/staff"
              className="w-12 h-12 bg-gray-100 hover:bg-gray-200 rounded-xl flex items-center justify-center transition-colors"
            >
              <ArrowLeft className="w-6 h-6 text-gray-600" />
            </Link>
            <div className="w-16 h-16 bg-gradient-to-br from-green-500 to-emerald-600 rounded-2xl flex items-center justify-center border-2 border-white shadow-lg">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full text-gray-800 px-4 py-3 bg-white border border-gray-200 rounded-xl focus:ring-2 focus:ring-green-500 focus:border-transparent outline-none"
                  required
                />
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    onChange={handleChange}
                    className="w-full text-gray-800 pl-12 pr-4 py-3 bg-white border border-gray-200 rounded-xl focus:ring-2 focus:ring-green-500 focus:border-transparent outline-none"
                    required
                  />
                </div>
              </div>
          ...
          <div className="relative backdrop-blur-lg bg-white rounded-2xl p-6 shadow-xl border-t-4 border-green-500 overflow-hidden">
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    onChange={handleChange}
                    className="w-full text-gray-800 pl-12 pr-4 py-3 bg-white border border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-transparent outline-none"
              ...
              <div className="bg-green-50 rounded-xl p-4 mt-6">
                <p className="text-sm text-green-800 font-semibold mb-1">Mã nhân viên</p>
                <p className="text-base font-mono font-bold text-green-900">{user.manvyt}</p>
                <p className="text-xs text-green-600 mt-1">Không thể thay đổi</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="backdrop-blur-lg bg-white/70 rounded-2xl p-6 shadow-xl border border-white/50">
          <div className="flex flex-wrap gap-4 justify-end">
            ...
            <button
              type="submit"
              disabled={loading}
              className="group flex items-center gap-2 px-8 py-3 bg-gradient-to-r from-green-600 to-emerald-600 text-white rounded-xl font-bold shadow-lg hover:shadow-xl hover:scale-105 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {loading ? (
                <>
